...
class New:
    ...
    def POST(self):
            ...
            imagenes = web.input(imagen={})

            filedir = 'static/files'
            filepath=imagenes.imagen.filename.replace('\\','/')
            filename = filepath.split('/')[-1]
            #copia archivo al servidor
            fout = open(filedir +'/'+ filename,'w')
            fout.write(imagenes.imagen.file.read())
            fout.close()
            imagen = filename

            if not form.validates():
                return render.new(form)
            model.new_post(form.d.producto,
                            form.d.descripcion,
                            form.d.existencias,
                            form.d.precio_compra,
                            form.d.precio_venta,
                            imagen)
            raise web.seeother('/')
